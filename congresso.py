...
def validar_telefone(telefone):
    # Verifica se o telefone contém apenas números e possui entre 10 e 11 dígitos
...
if st.session_state["botao_clicado"]:
    ...
    telefone = st.text_input("Telefone")  # Este campo será validado
...
if st.session_state["opcao_escolhida"] == "nao_associado":
    ...
    if st.button("ENVIAR (NÃO ASSOCIADO)"):
        if nome_completo_na and email_na and telefone_na:
            if not validar_telefone(telefone_na):
                st.error("Por favor, insira um telefone válido com DDD (exemplo: 11987654321).")
            elif "@" not in email_na:
                st.error("Por favor, insira um email válido.")
            else:
