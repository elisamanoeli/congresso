...
if "instituicao_selecionada" not in st.session_state:
    ...
    if st.button("ENVIAR", key="btn_enviar_associado"):
        if nome_completo and email and telefone:
            ...
            if not consultar_status_associado(nome_completo, status_selecionado):
                st.error(f"O nome {nome_completo} não corresponde a um associado com status {status_selecionado}. Verifique seus dados e tente novamente.")
